...
def imprimir_mapa_oculto(mapa: list, posicion_jugador: tuple):
    ...
    :param mapa: El mapa del juego.
    :return: El mapa con fila y columnas numeradas y los interrogantes.
        """
    #Uso un bucle para generar la primera línea numerada
    print("  " + " ".join(str(i + 1) for i in range(DIMENSIONES)))
    #Uso un contandor para generar el número de la fila antes de imprimir cada fila y celdas
    cont = 0
    for fila in mapa:
        cont += 1
        print(str(cont) + " " + " ".join([simbolo_celda(celda) for celda in fila]))
    

    """for i in range(DIMENSIONES):
        print((i + 1), end=" ")
        for j in range(DIMENSIONES):
            posicion_jugador = list(posicion_jugador)
            if posicion_jugador == [i, j]:
                print("J")
            else:
                celda = mapa[i][j]
                print(simbolo_celda(celda), end=" ")
        print()"""
...
def muestra_estado_mapa(mapa: list, posicion_jugador):
    """Muestra el mapa y la posición del jugador.
        :param mapa: El mapa a imprimir.
        :param posicion_jugador: La posición del jugador a mostrar.
        """

    imprimir_mapa_oculto(mapa, posicion_jugador)
    #Modifica la posición de forma visual acorde al mapa impreso.
    posicion_jugador = list(posicion_jugador)
    for i in range(len(posicion_jugador)):
        posicion_jugador[i] = posicion_jugador[i] + 1
...
def jugar():
    ...
    while movimiento != SALIR and resultado_movimiento != TESORO_ENCONTRADO:
        ...
        # Borra la consola cada vez que se ejecuta un movimiento.
        borrar_consola()
